...
def mnist_noniid(args, dataset, num_users, n_list, k_list):
    ...
    # Tổng số ảnh trong tập huấn luyện MNIST
    num_imgs = len(dataset.train_labels)
    
    dict_users = {}
    idxs = np.arange(num_imgs)
    labels = dataset.train_labels.numpy()

    # Sắp xếp nhãn và chỉ số tương ứng
    idxs_labels = np.vstack((idxs, labels))
    idxs_labels = idxs_labels[:, idxs_labels[1, :].argsort()]
    idxs = idxs_labels[0, :]
    
    # Tạo một từ điển chứa dữ liệu cho mỗi người dùng và danh sách các lớp
    classes_list = []
    ...
    for i in range(num_users):
        ...
        # Chọn ngẫu nhiên n lớp từ tất cả các lớp
        classes = random.sample(range(0, args.num_classes), n)
        classes = np.sort(classes)
        ...
        for each_class in classes:
            # Bắt đầu từ chỉ số của lớp và thêm k ảnh vào dữ liệu người dùng
            begin = np.where(labels == each_class)[0][0]
            begin = i * k_len + begin
            user_data = np.concatenate((user_data, idxs[begin : begin+k]), axis=0)
        
        dict_users[i] = user_data.astype(int)
        classes_list.append(classes)
...
# def mri_noniid(args, dataset, num_users, n_list, k_list):
#     """
#     Sample non-I.I.D client data from MNIST dataset
#     :param dataset:
#     :param num_users:
...
#     ### Labels in exel
#     labels_df = pd.read_csv('C:\\Users\\admin\\Documents\\Comvis2023\\data.csv', usecols=[17], skiprows=1)
#     idxs = np.arange(num_shards * num_imgs)
#     labels = labels_df.iloc[:len(idxs), 0].to_numpy()
...
def mri_noniid_lt(args, test_dataset, num_users, n_list, k_list, classes_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
...
def mri_noniid_unequal(dataset, num_users):
    """
    Sample non-I.I.D client data from MNIST dataset s.t clients
    have unequal amount of data
    :param dataset:
