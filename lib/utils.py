...
from sampling import mri_iid, mri_noniid, mri_noniid_lt, mri_noniid_unequal
from sampling import brain_iid, brain_noniid, brain_noniid_lt
import femnist
import MRI_brain
import numpy as np
...
trans_brain_train = transforms.Compose([transforms.RandomCrop(128, padding=4),
                                          transforms.RandomHorizontalFlip(),
...
def get_dataset(args, n_list, k_list):
    ...
    # day la them thu vien data ve nao
    #if args.datset == 'nao':
     #   al
    ...
    if args.dataset == 'mnist':
     ...
     # data nao mri viet tu day
        ...
        train_dataset = MRI_brain.MRI_BRAIN(args, data_dir, train=True, download=False,
                                        transform=apply_transform)
        test_dataset = MRI_brain.MRI_BRAIN(args, data_dir, train=False, download=False,
                                       transform=apply_transform)
        ...
        if args.iid:
            ...
            if args.unequal:
                ...
                # Chose euqal splits for every user
                user_groups, classes_list = mri_noniid(args, train_dataset, args.num_users, n_list, k_list)
                user_groups_lt = mri_noniid_lt(args, test_dataset, args.num_users, n_list, k_list, classes_list)
                classes_list_gt = classes_list
    # het  
    
    elif args.dataset == 'brain':
        train_dataset = datasets.ImageFolder(root='C:\\Users\\admin\\Documents\\Comvis2023\\FedProto\\data\\Brain_tumor\\Training', transform= trans_brain_train)
        test_dataset = datasets.ImageFolder(root='C:\\Users\\admin\\Documents\\Comvis2023\\FedProto\\data\\Brain_tumor\\Testing', transform= trans_brain_train)
